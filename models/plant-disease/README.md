...
A **transformers** detection model, not Ultralytics weights — leaf disease is
only published in that form, which is why `detector.py` grew a second model
family. Loaded via `HF_MODELS=plant=/models/plant-disease`.
...
**Scores run lower than COCO's.** A genuine tomato blight sits at 0.569, which
the global `MIN_CONFIDENCE=0.65` would have discarded before any rule saw it —
the detection would simply never have appeared. Hence `MODEL_CONFIDENCE`, and
this model's floor of 0.40.
